...
static void vibrator_enable(struct vibrator *haptic, int value)
{
	char mode;
	...
	if (value) {
		drv2604_read_reg(STATUS_REG);
			...
			if (mode != MODE_REAL_TIME_PLAYBACK) {
				...
				drv2604_set_rtp_val(haptic->level);
...
static void drv2604_worker(struct work_struct *work)
{
	struct vibrator *haptic;

	haptic = container_of(work, struct vibrator, work);
	vibrator_enable(haptic, !!haptic->level);
}

static void drv2604_haptic_close(struct input_dev *dev)
{
	struct vibrator *haptic = input_get_drvdata(dev);

	cancel_work_sync(&haptic->work);

	vibrator_off();
}
static int drv2604_haptic_play_effect(struct input_dev *dev, void *data,
...
static int drv2604_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		printk(KERN_ALERT "drv2604 on M3 probe failed");
		return -ENODEV;
	}
	...
	if ((status & DIAG_RESULT_MASK) == AUTO_CAL_FAILED) {
		printk(KERN_ALERT "drv2604 auto-calibration failed after %d attempts.\n", nCalibrationCount);
	} else {
		/* restore 0x1a */
	...
	switch (g_nDeviceID) {
	case DRV2605:
		printk(KERN_ALERT "drv2604 driver found: drv2605.\n");
		break;
	case DRV2604:
		printk(KERN_ALERT "drv2604 driver found: drv2604.\n");
		break;
	default:
		printk(KERN_ALERT "drv2604 driver found: unknown.\n");
		break;
	}
	...
	printk(KERN_ALERT "drv2604 on M3 probe succeeded");
	...
	int error;
	haptic = kzalloc(sizeof(struct vibrator), GFP_KERNEL);
	...
	input_dev->name = "drv2604-haptic";
	input_dev->id.version = 1;
	input_dev->dev.parent = &client->dev;
	input_dev->close = drv2604_haptic_close;
	input_set_drvdata(input_dev, haptic);
	input_set_capability(input_dev, EV_FF, FF_RUMBLE);

	error = input_ff_create_memless(input_dev, NULL,
				drv2604_haptic_play_effect);
	if (error) {
		dev_err(&client->dev,
	...
	INIT_WORK(&haptic->work, drv2604_worker);
	return 0;
